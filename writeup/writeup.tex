...
For an xl model with context length 512, ampere_sgemm_128x64_tn (matrix multiplication kernel operating on 128 rows × 64 columns in single precision) 
...
ampere_sgemm_64x32_sliced1x4_nn, but for both steps combined ampere_sgemm_128x64_tn still takes the most time.
...
I see elementwise operations, such as multiplication and addition, (at::native::elementwise_kernel and at::native::vectorized_elementwise_kernel) 
and reductions, such as mean and max (at::native::reduce_kernel).
...
For an xl model with context length 512, focusing on scaled\_dot\_product\_attention, most of the time (72\%) goes on softmax, where most work
is done by elementwise kernels, but over scaled\_dot\_product\_attention, 87\% of the time is taken by matrix multiplication kernels.
\end{enumerate}
