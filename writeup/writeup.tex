...
\begin{enumerate}[label=(\alph*)]
    ...
    the 2.7B model's inference was faster than that of the xl or even large model at small context window sizes.
...
Using an H-100 since on an A-100 we would run out of memory.

\begin{enumerate}[label=(\alph*)]
    
    \item \textbf{How do your memory timelines look like? Can you tell which stage is running based on the peaks you see?}
    
For the forward pass during inference, most memory has alaredy been allocated and we see small bursts.  The largest peaks corresponds to \texttt{softmax}.  Smaller ones correspond to other
transformer block operations, such as \texttt{SiLU}.
...
For the full step, we see a small spike for \texttt{zero\_grad} in the beginning, then a gradual increase in memory use during the forward pass (store activations with spikes for softmax, etc.).
During the backward pass, we deallocate activations while allocating / reusing memory for gradients.  Finally, the optimizer step consumes a lot of memory for 
the two AdamW moments per parameter.  
...
For the forward pass (inference), the peak memory usage expectedly grows with the context length.  
Optimizer dominates the training step in terms of memory consumption, but this difference is less pronounced as we increase the context length, so
for larger context windows we would expect the forward pass to require the most memory.  

    \input{tables/peak_memory.tex}
    
    \item \textbf{Does mixed-precision significantly affect memory usage?}
        
We instantiate the model in float32 and then use autocast for the forward pass, so we increase memory consumption during warmup - before taking measurements.  
Yes, most activations become smaller, but during inference we don't keep them around anyway.  During training, the memory footprint
of the forward pass is reduced with mixed precision; however, in our setup the peak memory usage is ultimately dominated by the optimizer step.

    \input{tables/peak_memory_mp.tex}
